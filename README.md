...
| 你是谁 | 怎么做 | 要不要部署 |
|--------|--------|-----------|
| **评委 / 体验用户** | 打开控制台 → **快速集成 → 在线试集成**，登录后点「立即评估」 | 否（用演示站点即可） |
| **Python 开发者** | `PYTHONPATH=. python ../examples/minimal_embed.py` 或 `from app.integration import evaluate` | 否 |
| **团队 HTTP 接入** | `POST /api/shield/evaluate`（用当前站点同源地址，勿写死 localhost） | 可选 |

详见 [INTEGRATION.md](./INTEGRATION.md)

**线上演示部署**见 [deploy/DEPLOY.md](./deploy/DEPLOY.md)（域名 agentshieldtop.xyz + 阿里云轻量服务器，与现有业务 nginx 共存）。
