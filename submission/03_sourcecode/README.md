...
**在线演示**：<http://114.215.209.144:8088>  
**健康检查**：<http://114.215.209.144:8088/health>  
**开源仓库**：<https://github.com/huang08666/agentShield>  
（域名 `agentshieldtop.xyz` 备案完成后可切换，见 [deploy/IP-PORT-ACCESS.md](./deploy/IP-PORT-ACCESS.md)）
...
| 你是谁 | 怎么做 | 要不要部署 |
|--------|--------|-----------|
| **评委 / 体验用户** | 打开 <http://114.215.209.144:8088> → 登录 → **快速集成 → 在线试集成** →「立即评估」 | 否 |
| **Python 开发者** | `git clone` 本仓库 → `from app.integration import evaluate`（见 [INTEGRATION.md](./INTEGRATION.md)） | 否 |
| **团队 HTTP 接入** | `POST http://114.215.209.144:8088/api/shield/evaluate`（或部署后使用你的同源地址） | 可选 |

详见 [INTEGRATION.md](./INTEGRATION.md) · 主流 Agent：[INTEGRATION-AGENTS.md](./INTEGRATION-AGENTS.md)

**自行部署**见 [deploy/DEPLOY.md](./deploy/DEPLOY.md)、[deploy/IP-PORT-ACCESS.md](./deploy/IP-PORT-ACCESS.md)。  
当前线上演示：**http://114.215.209.144:8088**（nginx :8088 + 后端 127.0.0.1:8001）。

## 代码同步（GitHub）

仓库：<https://github.com/huang08666/agentShield>
...
**国内云服务器往往无法 `git pull`（github.com:443 超时）**，请用下面方式之一更新线上：

```bash
# 推荐：在本机执行，rsync 到服务器并自动 deploy
bash deploy/sync-to-server.sh root@你的服务器IP

# 或在服务器配置 SSH 443 + GitHub 公钥后
bash deploy/pull-china.sh && sudo bash deploy/deploy.sh
```
